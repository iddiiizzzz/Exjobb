...
plt.hist(all_values, bins=10000, edgecolor='black') #bins=np.arange(all_values.max() + 2) - 0.5
plt.xlabel("Count Value")
plt.ylabel("Number of counts")
plt.title("Histogram of Count Matrix Values for all organisms from waste water samples")
plt.xticks(range(min(50, all_values.max() + 1)))  # Ensure discrete x-axis values
#plt.xlim([1,50])
plt.savefig("histograms/bilder/organisms_all/histogram_wastewater.png")
